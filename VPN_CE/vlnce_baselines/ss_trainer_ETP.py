...
from vlnce_baselines.common.utils import extract_instruction_tokens, load_scan2image, load_traj2locs, extract_visual_instr_tokens
...
class RLTrainer(BaseVLNCETrainer):
    def __init__(self, config=None):
        ...
        self.node2pix = json.load(open('data/datasets/trajectories/mp3d_Node2pix.json'))
        self.scan2images = load_scan2image('data/datasets/trajectories/mp3d_floors')
        self.traj2scanvps = load_traj2locs('data/datasets/trajectories/R2R_{}_enc.json')
    ...
    def rollout(self, mode, ml_weight=None, sample_ratio=None):
        ...
        observations, batch_vis_instrs, batch_indexes, batch_floors = extract_visual_instr_tokens(observations, self.node2pix,
                                                                      self.scan2images, self.traj2scanvps, self.device, mode)
        ...
        # )
        all_txt_embeds, all_txt_masks = self.policy.net(
                mode = 'vis_instr',
                vis_instrs = batch_vis_instrs,
        ...
        # # observations = extract_instruction_tokens(observations, self.config.TASK_CONFIG.TASK.INSTRUCTION_SENSOR_UUID,
        # #                                           max_length=instr_max_len, pad_id=instr_pad_id)
        # observations, batch_vis_instrs, batch_indexes, batch_floors = extract_visual_instr_tokens(observations,
        #                                                                                           self.node2pix,
        #                                                                                           self.scan2images,
        #                                                                                           self.traj2scanvps)
        # batch = batch_obs(observations, self.device)

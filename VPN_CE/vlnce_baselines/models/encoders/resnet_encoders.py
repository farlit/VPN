...
class VitEncoder(nn.Module):
    def __init__(self, device):
        ...
        for param in self.model.parameters():
            param.requires_grad_(False)
        self.model.eval()
        self.rgb_transform = torch.nn.Sequential(transforms.ConvertImageDtype(torch.float),
    ...
    def forward(self, observations):
        ...
        rgb_observations = self.rgb_transform(rgb_observations)
        # print(rgb_observations.shape)
        embeddings = self.model.embeddings(rgb_observations)
        # print(embeddings.shape)
        outputs = self.model.encoder(embeddings)['last_hidden_state']
        output = outputs[:, 0 ,:]

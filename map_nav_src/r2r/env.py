...
class R2RNavBatch(object):
    ...
    def __init__(
        ...
        test_vp_dir = None, agent_enhance=False
    ):
        ...
        if self.name == 'test':
            self.test_vps = {}
            folder_names = [name for name in os.listdir(test_vp_dir)
                            if os.path.isdir(os.path.join(test_vp_dir, name)) and name.isdigit()]
            folder_ids = sorted([int(name) for name in folder_names])
            for folder_id in folder_ids:
                folder_path = os.path.join(test_vp_dir, str(folder_id))
                img_dict = {}
                png_files = [fname for fname in os.listdir(folder_path)
                            if fname.endswith('.png') and fname[:-4].isdigit()]
                png_files = sorted(png_files, key=lambda x: int(x[:-4]))  # 按数字排序
                for fname in png_files:
                    img_idx = int(fname[:-4])
                    img_path = os.path.join(folder_path, fname)
                    img = cv2.imread(img_path)
                    if img is not None:
                        img_dict[img_idx] = img
                    else:
                        print(f"Warning: Failed to read {img_path}")
                self.test_vps[folder_id] = img_dict
        self.agent_enhance = agent_enhance

    def _get_gt_trajs(self, data):
        gt_trajs = {
            x['path_id']: (x['scan'], x['path']) \
    ...
    def _get_obs(self):
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            ...
            ob = {
                ...
                'instr_encodings': item['instr_encodings'],
                'gt_path' : item['path'],
    ...
    def reset(self, **kwargs):
        ...
        if self.agent_enhance and self.name == 'aug':
            headings = [np.random.uniform(0, 2*np.pi) for _ in self.batch]
        else:
    ...
    def eval_metrics(self, preds):
        ...
        for item in preds:
            ...
            scan, gt_traj = self.gt_trajs[path_id]
            traj_scores = self._eval_item(scan, traj, gt_traj)
            for k, v in traj_scores.items():
                metrics[k].append(v)
            metrics['path_id'].append(path_id)

import os
import sys
sys.path.append('/root/mount/Matterport3DSimulator/tasks/VPN/map_nav_src')
import json
import time
import numpy as np
...
def build_dataset(args, rank=0, is_test=False):
    ...
    if args.aug is not None:
        if args.hm3d is not None:
            aug_instr_data = construct_instrs(
                args.anno_dir, args.dataset, ['train', args.aug, args.hm3d],
                tokenizer=args.tokenizer, max_instr_len=args.max_instr_len,
                is_test=is_test
            )
        else:
            aug_instr_data = construct_instrs(
                args.anno_dir, args.dataset, ['train', args.aug],
                tokenizer=args.tokenizer, max_instr_len=args.max_instr_len,
                is_test=is_test
            )
    else:
    ...
    aug_env = dataset_class(
        ...
        agent_enhance=args.agent_enhance
    )
    ...
    for split in val_env_names:
        ...
        val_env = dataset_class(
            ...
            test_vp_dir=args.test_vp_dir
        )  # evaluation using all objects
        val_envs[split] = val_env

    return aug_env, val_envs
...
def main():
    ...
    train_env, val_envs = build_dataset(args, rank=rank, is_test=args.test)

    if not args.test:
        train(args, train_env, val_envs, aug_env=None, rank=rank)
    else:

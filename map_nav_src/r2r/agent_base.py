...
from utils.data import load_scan2image_mp3d, load_scan2image_hm3d
...
class BaseAgent(object):
    ...
    def get_results(self, detailed_output=False):
        ...
        for k, v in self.results.items():
            output.append({'path_id': k, 'trajectory': v['path']})
            if detailed_output:
                output[-1]['details'] = v['details']
        return output

    def rollout(self, **args):
        ''' Return a list of dicts containing path_id:'xx', path:[(viewpointId, heading_rad, elevation_rad)]  '''
        ...
        if iters is not None:
            ...
            for i in range(iters):
                for traj in self.rollout(**kwargs):
                    self.loss = 0
                    self.results[traj['path_id']] = traj
        else:   # Do a full round
            while True:
                for traj in self.rollout(**kwargs):
                    if traj['path_id'] in self.results:
                        looped = True
                    else:
                        self.loss = 0
                        self.results[traj['path_id']] = traj
...
class Seq2SeqAgent(BaseAgent):
    ...
    def __init__(self, args, env, rank=0):
        ...
        # Viusal instructions loading
        self.node2pix = {**json.load(open(os.path.join(args.anno_dir, 'mp3d_Node2pix.json'))), **json.load(open(os.path.join(args.anno_dir, 'hm3d_Node2pix.json')))}
        self.scan_to_images = {**load_scan2image_mp3d(args.mp3d_floor_dir), **load_scan2image_hm3d(args.hm3d_floor_dir)}

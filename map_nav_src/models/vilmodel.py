...
#
#     def group_average(self, x, batch_indexes):
#         N, T, D = x.shape
#         device = x.device
#         num_groups = batch_indexes.max().item() + 1
#         x_sum = torch.zeros((num_groups, T, D), device=device)
#         x_count = torch.zeros(num_groups, 1, 1, device=device)
#         x_sum.index_add_(0, batch_indexes, x)
#         x_count.index_add_(0, batch_indexes, torch.ones_like(x[:, :1, :1]))
#         x_avg = x_sum / x_count
#         return x_avg
#
#     def forward(self, x, batch_indexes, batch_floors):
#         x = self.vit._process_input(x)
#         n = x.shape[0]
#         batch_class_token = self.vit.class_token.expand(n, -1, -1)
#         x = torch.cat([batch_class_token, x], dim=1)
#         x = self.vit.encoder(x)
#         x = self.group_average(x, batch_indexes)
#         return x
...
class InstrEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.floor_embedding = nn.Embedding(2, config.hidden_size)
    ...
    def generate_floor_labels(self, batch_indexes):
        floor_labels = torch.zeros_like(batch_indexes)
        same_as_prev = torch.cat([
            torch.tensor([False], device=batch_indexes.device),
            batch_indexes[1:] == batch_indexes[:-1]
        ])
        floor_labels[same_as_prev] = 1
        return floor_labels

    def forward(self, x, batch_indexes, batch_floors):
        batch_floors = self.generate_floor_labels(batch_indexes)
        floor_embs = self.floor_embedding(batch_floors)
        x = self.model.embeddings(x)
        x = self.model.encoder(x)['last_hidden_state']
        x = x + floor_embs.unsqueeze(1)

        unique_indices, counts = torch.unique(batch_indexes, return_counts=True)
        num_groups = len(unique_indices)
        T = x.size(1)
        D = x.size(2)
        max_repeats = counts.max().item()
        max_group_len = max_repeats * T

        device = x.device
        padded_groups = torch.zeros((num_groups, max_group_len, D), device=device)
        masks = torch.zeros((num_groups, max_group_len), device=device)
        for i, idx in enumerate(unique_indices):
            group_mask = (batch_indexes == idx)
            group_x = x[group_mask]  # [M, T, D]
            M = group_x.size(0)
            concat_x = group_x.reshape(-1, D)  # [M*T, D]
            valid_len = M * T
            padded_groups[i, :valid_len] = concat_x
            masks[i, :valid_len] = 1

        # x = self.group_average(x, batch_indexes)
        return padded_groups, masks
...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.vis_instr_encoder = InstrEncoder(config)
        ...
        #         v.requires_grad = False
        if config.fix_lang_embedding or config.fix_local_branch:
            for k, v in self.vis_instr_encoder.named_parameters():
                v.requires_grad = False
    ...
    def forward_vis_instr(self, batch_vis_instrs, batch_indexes, batch_floors):
        instr_embeds, masks = self.vis_instr_encoder(batch_vis_instrs, batch_indexes, batch_floors)
        return instr_embeds, masks
    ...
    def forward(self, mode, batch, **kwargs):
        if mode == 'language':
            ...
            vis_instr_embeds, masks = self.forward_vis_instr(batch['batch_vis_instrs'], batch['batch_indexes'],
                                                      batch['batch_floors'])
            return vis_instr_embeds, masks
